...
// Write code to detect collision between player arrow and opponent
  for(var i = 0; i<playerArrows.length; i++){
    var base_collision = Matter.SAT.collides(
      playerArrows[i].body, 
      computerBase.body
    );
    var computer_collision = Matter.SAT.collides(
      playerArrows[i].body, 
      computer.body
    );
    var archer_collision = Matter.SAT.collides(
      playerArrows[i].body, 
      computerArcher.body
    );
   
      if(
        base_collision.collided ||
        computer_collision.collided||
        archer_collision.collided
      )
        {
...
function handleComputerArrowCollision() {
  ...
  for(var i = 0; i< computerArrows.length; i++){
    ...
    var player_collision = Matter.SAT.collides(
      computerArrows[i].body, 
      player.body
    );
    var archer_collision = Matter.SAT.collides(
